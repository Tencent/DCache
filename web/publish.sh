...
cp -rf /Users/jarod/.nvm/versions/node/v16.13.0/lib/node_modules/@tars/node-agent tars_nodejs
...
curl --progress-bar ${TARS_K8S_WEB_HOST}/pages/server/api/upload_and_publish?ticket=${TARS_TOKEN} -Fsuse=@${TARGET}.tgz -Fapplication=${APP} -Fmodule_name=${TARGET}  -Fcomment=upload
